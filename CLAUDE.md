...
- `spacetimedb/src/index.ts` — single-file module: schema, the quiz engine
  (phase timers, scoring, the answer key), the attention tally, accounts,
  the championship hook. The module entrypoint may ONLY
...
- `spacetimedb/src/bank.ts` is GENERATED by `spacetimedb/gen-bank.mjs` from
  ...
  Norwegian: 1431 questions over 44 topics in `20-*` … `63-*`. It runs before every build/publish (package
...
- ANSWERS ARE SECRET until the reveal: `player` is a PUBLIC table, so writing
  ...
  the `pick` row and RE-STAMPS `answeredAt` (so the fastest-finger line names
  the answer they stood behind, not a throwaway first tap), and only the
...
- Quiz flow: `startQuiz` → PH_INTRO → per question PH_ANSWER (`openQuestion`
  puts topic, difficulty, text and the shuffled options up together; the key
  stays hidden, `qCorrect` = NO_ANSWER) → PH_RESULT (`qCorrect` filled,
  `settleQuestion` scores it, writes `entry` rows) → … → `finishQuiz`
...
- Scoring: flat. START_SCORE 0, POINTS_PER_CORRECT 10 for a correct answer
  and nothing for anything else — no stakes, no difficulty weighting, no
  speed or streak bonus, and nobody ever loses points. Mirrored for display
  in `client/src/config.ts` — keep in sync. Betting is GONE: phase 2
  (PH_BETTING), `place_stake`, the odds and the landlord's tab are all
  retired, but their append-only columns stay behind at zero
  (`player.stake`, `player.tabs`, `lobby.qPayoutPct`, `lobby.betSecs`), and
  `player.credits` is now simply the running score. The phase numbers of
  what survived (PH_ANSWER 3, PH_RESULT 4, PH_DONE 5) are left alone.
...
- Championship hook: `lobby.championshipLeg`, `create_championship_room`
  ...
  `{questions, answerSecs}` — a legacy `betSecs` is ignored),
  `claimChampionshipHost` in `join_pub`,
